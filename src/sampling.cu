...
__host__ __device__ void sampleRandomLight(Geom* geo, int num_geoms, Volume* volumes, const Light* lights, int num_lights, const Material* materials, glm::vec3 intersect, glm::vec3 normal, glm::vec3 wo, const Material& m, thrust::default_random_engine& rng, glm::vec3& Li) {
    ...
    Ray shadowRay;
    shadowRay.origin = intersect + normal * BOUNCE_EPS;
    shadowRay.direction = wi;

    int hit_geom_index;
    float t_min;
    glm::vec3 shadow_intersect, shadow_normal; bool hitVolume = false;
    sceneIntersectionTest(geo, volumes, shadowRay, num_geoms, rng, hit_geom_index, t_min, shadow_intersect, shadow_normal, hitVolume);
